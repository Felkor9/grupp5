...
exports.createResor = async (req, res) => {
  ...
  try {
    await resorService.createResor(
      resorLand,
      resorDatum,
      resorLangd,
      resorPris
...
exports.getResaDestination = async (req, res) => {
  const { id } = req.params;

  try {
    const resor = await resorService.getResaDestination(id);
    res.json({ resor });
  } catch (error) {
    return res.status(500).json({
      error: error.message,
...
exports.getResorDestination = async (req, res) => {
  try {
    const resor = await resorService.getResorDestination();
    res.json({ resor });
  } catch (error) {
    return res.status(500).json({
      error: error.message,
